...
const GraphWorker = () => {
  ...
  useEffect(() => {
    ...
    if (graphContainer) {
       ...
       */
      const renderGoals = (goals, graph: Graph, source: Cell | null = null) => {
      ...
      const renderLegend = (graph: Graph): Cell => {
        ...
        const startX =
          -graph.view.translate.x + graph.view.graphBounds.width + 30;
       ...
       */
      const layoutFunctions = (graph: Graph) => {
        const layout = new GoalModelLayout(
          graph,
          VERTICAL_SPACING,
          HORIZONTAL_SPACING
        );
        layout.execute(graph.getDefaultParent(), rootGoal as Cell);
      };
       ...
       * functional goals.
       */
      const associateNonFunctions = (graph: Graph) => {
    ...
    graph.batchUpdate(() => {
      const vertex1 = graph.insertVertex(
        ...
        SYMBOL_WIDTH,
        SYMBOL_HEIGHT,
        { shape: PARALLELOGRAM_SHAPE, fontColor: "black" }
      );
 ...
 */
  ...
  const renderFromXML = (xml: string) => {
    ...
    let elt = doc.documentElement.firstChild as Element;
    const cells = [];

    while (elt) {
      console.log(elt);
      cells.push(codec.decode(elt));
      elt = elt.nextSibling as Element;
    }

    const graph = graphRef;
    if (!graph) return;

    graph.addCells(cells, null, null, null, null);
  };
      ...
      <Row>
        ...
        >
          <Button
            ...
            size="sm"
            className="non-draggable dragger"
            id="addgoalbtn"
          >
            Add Goal
          </Button>
